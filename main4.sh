...
ask_direct_login()
{
    read -p "File User-4 Data already exists, Would you like to take all the inputs from it? (y/n): " response;
    if [[ "$response" == 'y' ]]; then
    source $LOCALDIR/.usercreds4
    send_otp;
    ask_playlist_type;
    main;
...
{
    check_repo=$(curl -i -s -H "Authorization: token $git_token"     https://api.github.com/user/repos | grep 'Tata-Play-IPTV-Playlist-4-Daily-Generator') || true
...
ask_user_to_select()
{
    printf "\n Repo named 'Tata-Play-IPTV-Playlist-4-Daily-Generator' already exists, What would you like to perform? \n\n"
...
{
    if [[ $selection == '1' ]]; then
    dir="$(curl -s "https://$git_token@raw.githubusercontent.com/$git_id/Tata-Play-IPTV-Playlist-4-Daily-Generator/main/.github/workflows/Tata-Play-IPTV-Playlist-4-Daily-Workflow.yml" | perl -p -e 's/\r//g' | grep 'gist' | sed 's/.*\///g')"
...
create_gist()
{
    ...
    gh repo create Tata-Play-IPTV-Playlist-4-Daily-Generator --private -y >> /dev/null 2>&1 || true
    fi
}
...
main()
{
    ...
    cat $LOCALDIR/dependencies/Tata-Play-IPTV-Playlist-4-Daily-Workflow.yml | envsubst > Tata-Play-IPTV-Playlist-4-Daily-Workflow.yml
    cd ../..
    ...
    git remote add origin "https://$git_token@github.com/$git_id/Tata-Play-IPTV-Playlist-4-Daily-Generator.git"
    dynamic_push;
    ...
    save_creds;
    tput setaf 51; echo "Successfully created your new private repo." && printf "Check your new private repo here: ${NC}https://github.com/$git_id/Tata-Play-IPTV-Playlist-4-Daily-Generator\n" && tput setaf 51; printf "Check Your Playlist URL here: ${NC}https://gist.githubusercontent.com/$git_id/$dir/raw/TataPlayPlaylist.m3u \n"
    if [[ "$selection" == '2' ]]; then tput setaf 51; echo -e "Check your other playlist branch here: ${NC}https://github.com/$git_id/Tata-Play-IPTV-Playlist-4-Daily-Generator/tree/$dir"; fi
